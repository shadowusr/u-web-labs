...
$success = false;
$statement = $db->prepare('SELECT id FROM museums;', [PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL]);
$statement->execute();
if ($statement->rowCount() >= 20) {
    $status = 'Вы можете сохранять не более 20 записей.';
} elseif (empty($newItem['name']) || empty($newItem['city']) || empty($newItem['rating'])) {
    $status = 'Не было передано один или несколько входных параметров.';
} elseif (!is_string($newItem['name']) || mb_strlen($newItem['name']) > 128 || !is_string($newItem['city']) || mb_strlen($newItem['city']) > 128 || !is_numeric($newItem['rating'])) {
    $status = nl2br("Один или несколько параметров некорректны. Требования:\n- name и city должны быть непустыми строками, не превышающими длину в 128 знаков.\n- rating должен быть числом.");
} else {
    $sql = 'INSERT INTO museums (name, city, rating) VALUES (:name, :city, :rating)';
...
HEREDOC;
        ...
        <div class="row">
            <div class="col-12" >
                ...
                <p data-aos="fade-up">Исходный код страницы доступен на <a href="https://github.com/shadowusr/u-web-labs/tree/master/resources/views/tasks/task-7-museums-create.php">GitHub</a>.</p>
            </div>
        </div>
    </div>
</section>
