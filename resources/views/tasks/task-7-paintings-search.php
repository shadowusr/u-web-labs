...
$sql = 'SELECT * FROM paintings LEFT JOIN (SELECT id AS museum_id, name AS museum_name FROM museums) museums ON paintings.museum_id = museums.museum_id';
$searchParams = [];
$useAnd = false;
$insertWhere = true;
if (!empty($_GET['name'])) {
...
if (!empty($_GET['museum_id']) && is_numeric($_GET['museum_id'])) {
    if ($insertWhere) {
        $insertWhere = false;
        $sql .= ' WHERE';
    }
    ...
    $sql .= ' paintings.museum_id = :museum_id';
    $searchParams['museum_id'] = $_GET['museum_id'];
}
...
$paintings = escapeObjects($statement->fetchAll(PDO::FETCH_ASSOC));
?>
...
HEREDOC;
        ...
        <div class="row">
            <div class="col-12" >
                ...
                <p data-aos="fade-up">Исходный код страницы доступен на <a href="https://github.com/shadowusr/u-web-labs/tree/master/resources/views/tasks/task-7-paintings-search.php">GitHub</a>.</p>
            </div>
        </div>
    </div>
</section>
