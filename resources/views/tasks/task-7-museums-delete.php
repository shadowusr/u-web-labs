...
$paintingsCount = $db->query('SELECT id FROM paintings WHERE museum_id = ' . (int)$_GET['id'])->rowCount();

$success = false;
if (empty($_GET['id'])) {
    $status = 'Не было передано один или несколько входных параметров.';
} elseif (!is_numeric($_GET['id']) || !in_array($_GET['id'], $museumIds) || $paintingsCount > 0) {
    ...
    $sql = 'DELETE FROM museums WHERE id = ' . (int)$_GET['id'];
...
HEREDOC;
        ...
        <div class="row">
            <div class="col-12" >
                ...
                <p data-aos="fade-up">Исходный код страницы доступен на <a href="https://github.com/shadowusr/u-web-labs/tree/master/resources/views/tasks/task-7-museums-delete.php">GitHub</a>.</p>
            </div>
        </div>
    </div>
</section>
